...
def compute_metrics(
    ...
    # np.min across axis=1 gives the minimum equity seen in each path
    path_minimums = np.min(equity_paths, axis=1)
    ruin_count: int = int(np.sum(path_minimums <= ruin_floor))
    ...
    worst_drawdown_across_paths: float = float(np.max(drawdown_matrix))
