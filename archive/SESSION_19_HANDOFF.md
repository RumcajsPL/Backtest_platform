...
```
configs\strategies\wbws\wbws_strategy.yaml #Legacy config file, New architecture shares majority of setting by New architecture config template is yet to develop
...
src\strategies\specific\filters/        ← whole directory
```
...
```
> **Pre-confirmed P0 issues** (from real-data E2E test run — investigate in Chapters 4 & 5):
> - P0-1: Core mode trade simulation 11% slower than Debug mode and generally is sill slow. Integration to check especially SpreadManager
> - P0-2: Cache hit rate 50% on hot runs — never reaching 100%
---
## 🎯 SESSION 19 GOAL
...
- 60 min — code scan (7 chapters, ~8 min each) - follow architecture pipeline data->signal->filter...
...
**Files**:
```
configs\strategies\wbws\wbws_strategy.yaml (config file)
src\config\config_schema.py        (StrategyConfig)
src\utils\structured_logger.py (logger for E2E logging)
```
**What to look for**:
...
src/strategies/contracts/signal_contracts.py
src/strategies/specific/modules/signal_generator.py
src\indicators\wbws_trigger.py #optional to check - raw signal specific to wbws strategy (not generic part of archiitecture)
```
**What to look for**:
...
```
src/strategies/contracts/filter_contracts.py
src/strategies/specific/filters/      ← enumerate all files
src/strategies/specific/modules/filter_pipeline.py
```
**What to look for**:
**Contracts**:
...
- [ ] Focus on spread integration and processing speed => perf?
...
### Code Scan planning
**Example to retake**
### Step 8.1: Code Scan ⏳ SESSIONS 19-?
- [ ] Phase PO
...
1. ...
### Implementation Plan ⏳ SESSIONS 20-?
- [ ] Phase PO Fix Immediately
- [ ] P0 ... 
- [ ] P1 ...
...
| TradeAnalytics | `trade_analytics.py` | 14-16 | - |
| ReportGenerator | `report_generator.py` | 17-18 | -|
**Total Phase 5-7 tests**: 374  
**Known test gap**: Phases 1-4 modules have no dedicated unit tests in `tests/migration/` — scan to confirm and document, only integration tests.
