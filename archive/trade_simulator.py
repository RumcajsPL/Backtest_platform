...
class TradeSimulator:
    ...
    import numpy as np  # Add this at the top of your file if not already present
    ...
    def _check_exits_with_ltf_ohlc(self, strategy_timestamp: pd.Timestamp, 
        ...
        # Get array of lows and highs for vectorized operations
        ltf_lows = ltf_bars['low'].values
        ltf_highs = ltf_bars['high'].values
        ltf_timestamps = ltf_bars.index
        ...
        for open_trade in list(self.trade_tracker.get_open_trades()):
            ...
            if open_trade['direction'] == 'BUY':
                # Check for SL hit (low <= SL)
                sl_hits = np.where(ltf_lows <= open_trade['sl_price'])[0]
                if len(sl_hits) > 0:
                    exit_idx = sl_hits[0]  # First hit
                    exit_price = min(ltf_lows[exit_idx], open_trade['sl_price'])
                    exit_info = {'exit_reason': 'STOP_LOSS', 'exit_price': exit_price}
                
                # Check for TP hit (high >= TP) - only if SL not hit
                if exit_info is None:
                    tp_hits = np.where(ltf_highs >= open_trade['tp_price'])[0]
                    if len(tp_hits) > 0:
                        exit_idx = tp_hits[0]
                        exit_price = min(ltf_highs[exit_idx], open_trade['tp_price'])
                        exit_info = {'exit_reason': 'TAKE_PROFIT', 'exit_price': exit_price}
            else:  # SELL
                # Check for SL hit (high >= SL)
                sl_hits = np.where(ltf_highs >= open_trade['sl_price'])[0]
                if len(sl_hits) > 0:
                    exit_idx = sl_hits[0]  # First hit
                    exit_price = max(ltf_highs[exit_idx], open_trade['sl_price'])
                    exit_info = {'exit_reason': 'STOP_LOSS', 'exit_price': exit_price}
                
                # Check for TP hit (low <= TP) - only if SL not hit
                if exit_info is None:
                    tp_hits = np.where(ltf_lows <= open_trade['tp_price'])[0]
                    if len(tp_hits) > 0:
                        exit_idx = tp_hits[0]
                        exit_price = max(ltf_lows[exit_idx], open_trade['tp_price'])
                        exit_info = {'exit_reason': 'TAKE_PROFIT', 'exit_price': exit_price}
            
            if exit_info:
                ltf_timestamp = ltf_timestamps[exit_idx]
                ltf_row = ltf_bars.iloc[exit_idx]
                if open_trade['direction'] == 'BUY':
