...
> - v2.0.0 (2026-03-04): Block 9A/9B updates throughout — see end of document for full diff
---
...
detected. Default for both: `0.15`. This will be fixed in Block 9C (B9A-003).
...
| 1 | Random Search | 🟡 Stub | Logs "not yet implemented". Checkpoint **is advanced**. |
| 2 | MC Pre-Filter | 🟡 Stub | Logs "not yet implemented". Checkpoint **is advanced**. |
| 3 | GA Evolution | 🟡 Stub | Logs "not yet implemented". Checkpoint **is advanced**. |
...
> resume, no stub stage re-runs. Prior to Block 9A, Stage 1 re-ran on every resume
> (B9A-002 fixed).
...
> to confirm the ≤200s target (and identify any remaining optimisations) is planned.
...
```yaml
walk_forward:
  enforce_oos_gate: false   # Currently has NO EFFECT — see §9.1
```
...
- OOS gate was triggered — **NOTE: currently non-functional, see §9.1**
...
> pipeline run. See §9.2 for WFO sigmoid scale note.
...
If an old run has checkpoint `RUN_INITIALISED` and you suspect it predates the B9A-002
fix: safe to resume — Stage 1 will run once and advance its checkpoint correctly.
...
Expectancy is in `expectancy_points`.
...
### 11.3 Previously Documented Limitations
...
| spike detected but not flagged in verdict (or vice versa) | B9A-003: `spike_threshold` differs between config locations | Ensure `sensitivity.spike_threshold` matches `verdict_sensitivity_spike_threshold` in scenario — see §1.6 |
...
Trading YAMLs embed `backtester_metadata` block containing: `run_id`, `config_hash`,
`verdict`, `wfo_consistency_score`, `mc_ruin_probability`, `sensitivity_spike_detected`,
and all 5 seeds.

---

## v2.0.0 Change Summary (Block 9A/9B)

- **§1.6 added**: spike_threshold consistency check — operator must keep config dict and
  ScenarioProfile in sync until B9A-003 is fixed
- **§3**: Stage table updated — stubs now show "checkpoint is advanced" (B9A-002 fix)
  and B9A-002 callout added; resume behaviour note updated
- **§4.2**: OPT-01 note updated — applied but ≤200s target not yet confirmed,
  re-analysis planned
- **§4.3**: TIMING SUMMARY note added (B8-008 — covers Stages 5–7 only)
- **§5**: spike_threshold reference points to §1.6; `enforce_oos_gate` note updated
- **§6**: "All metrics in pips/points" note added to Stage 5 and Stage 6
- **§11.2**: Rewritten — removed incorrect currency framing; now correctly states
  pips/points throughout; calibration guidance updated accordingly
- **§11.3**: B8B-018 marked Fixed; B8C-001 marked Fixed; B9A-002 marked Fixed;
  B9A-003 (spike_threshold dual-source) added as open
- **Appendix A**: B9A-002, B9B-003, B9A-003 error patterns added; B8B-018 entry removed
  (fixed); B8B-012 entry updated for pips/points framing