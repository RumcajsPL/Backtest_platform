...
**Success Criteria**: 
- All filters & FilterPipeline uses typed FilterResult contracts (if reasonable).
- New FilterPipeline outputs match old system (signal parity test).
- Performance ≤110% of baseline (benchmark on 3-month dataset).
...
- Review existing filters in `src/strategies/core/filters/` (time_manager.py + 10 technical: ADX, Bollinger, CCI, Choppiness, DPO, MA, MACD, Pivot, RSI, Supertrend).
Whatever relevan (non exhaustive)
...
- Run parallel tests: Compare old `core/filter_pipeline.py` vs new `specific/filter_pipeline.py` outputs on 3-month dataset.
- Performance benchmark: measure execution time (target: <110% baseline).
...
Test scripts:
tests\migration\test_signal_generator.py
Migration documentation:
docs\migration\DATALOADER_AUDIT.md
docs\migration\DECISION_LOG.md
docs\migration\MIGRATION_PLAN.md
docs\migration\PHASE_2_ANALYSIS.md
docs\migration\PHASE_3_PLAN.md
docs\migration\PROJECT_CHARTER.md
docs\migration\SESSION_1_SUMMARY.md
docs\migration\SESSION_2_SUMMARY.md
docs\migration\SESSION_LOG.md
docs\migration\SIGNAL_GENERATOR.md
---
## Risks & Mitigations
